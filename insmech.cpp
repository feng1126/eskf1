...
void INSMech::insMech(const State& pvapre, State& pvacur, const filterMessage& imupre, const filterMessage& imucur) {
...
void INSMech::velUpdate(const State& pvapre, State& pvacur, const filterMessage& imupre, const filterMessage& imucur)
{
    ...
    wen_n << pvapre.v[1] / (rmrn[1] + pvapre.pos[2]), -pvapre.v[0] / (rmrn[0] + pvapre.pos[2]),
        -pvapre.v[1] * tan(pvapre.pos[0]) / (rmrn[1] + pvapre.pos[2]);
    double gravity = Earth::gravity(pvapre.pos);

    // 旋转效应和双子样划桨效应
    // rotational and sculling motion
    temp1 = (imucur.gyro * dt).cross(imucur.acc * dt) / 2;
    temp2 = (imucur.gyro * dt).cross(imucur.acc * dt) / 12;
    temp3 = (imucur.acc * dt).cross(imucur.gyro * dt) / 12;

    // b系比力积分项
    // velocity increment due to the specific force
    d_vfb = imucur.acc * dt + temp1 + temp2 + temp3;
    ...
    d_vgn = (gl - (2 * wie_n + wen_n).cross(midvel)) * dt;

    // 速度更新完成
    // velocity update finish
    pvacur.v = pvapre.v + d_vfn + d_vgn;
}

void INSMech::posUpdate(const State& pvapre, State& pvacur, const filterMessage& imupre, const filterMessage& imucur)
{
    ...
    //midvel = (pvacur.v + pvapre.v) / 2;
    //midpos = pvapre.pos + Earth::DRi(pvapre.pos) * midvel * dt / 2;
    ...
    //qne = Earth::qne(pvapre.pos);
    //qne = qee * qne * qnn;
    //pvacur.pos[2] = pvapre.pos[2] - midvel[2] * dt;
    //pvacur.pos = Earth::blh(qne, pvacur.pos[2]);

    pvacur.p = pvapre.p +  dt * pvacur.v ;


}

void INSMech::attUpdate(const State& pvapre, State& pvacur, const filterMessage& imupre, const filterMessage& imucur) {
